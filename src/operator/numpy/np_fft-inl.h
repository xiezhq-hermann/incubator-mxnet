...
class NPFFTOp : public Operator {
 public:
  explicit NPFFTOp(NPFFTParam p) {
    this->param_ = p;
    init_cufft_ = false;
    dim_ = 0;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    for (size_t idx=0; idx < num_compute; ++idx) {
      cufftReal* in_tmp = const_cast<cufftReal*>(
        reinterpret_cast<const cufftReal*>(data.Slice(idx*param_.compute_size,
                                                 idx*param_.compute_size+param_.compute_size).dptr_));
      cufftComplex* out_tmp = reinterpret_cast<cufftComplex*>(out.dptr_ + idx*stride_);
      CHECK_EQ(cufftExecR2C(plan, in_tmp, out_tmp), CUFFT_SUCCESS);
    }
    cufftDestroy(plan);
    ...
    if (remain_num > 0) {
      ...
      cufftComplex* in_tmp = const_cast<cufftComplex*>(
        reinterpret_cast<const cufftComplex*>(data.Slice(
          num_compute*param_.compute_size, num_compute*param_.compute_size+remain_num).dptr_));
      cufftComplex* out_tmp = reinterpret_cast<cufftComplex*>(out.dptr_ + num_compute*stride_);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Tensor<xpu, 1, DType> workspace =
            ctx.requested[fft::kTempSpace].get_space_typed<xpu, 1, DType>(
                Shape1(param_.compute_size*dim_), s);
    Tensor<xpu, 2, DType> complex_data = Tensor<xpu, 2, DType>(workspace.dptr_,
                                              Shape2(param_.compute_size, dim_), s);
    ...
    for (size_t idx = 0; idx < num_compute; ++idx) {
      cufftComplex* in_tmp = const_cast<cufftComplex*>(
        reinterpret_cast<const cufftComplex*>(grad.dptr_ + idx*stride_));
    ...
    if (remain_num > 0) {
      ...
      complex_data = Tensor<xpu, 2, DType>(workspace.dptr_,
                                              Shape2(remain_num, dim_), s);

      cufftComplex* in_tmp = const_cast<cufftComplex*>(
        reinterpret_cast<const cufftComplex*>(grad.dptr_ + num_compute*stride_));
      cufftComplex* out_tmp = reinterpret_cast<cufftComplex*>(complex_data.dptr_);
...
Operator* CreateNPOp(NPFFTParam param, int dtype);

#if DMLC_USE_CXX11
class NPFFTProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    NPFFTProp* fft_sym = new NPFFTProp();
